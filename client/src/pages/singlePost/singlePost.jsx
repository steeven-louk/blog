...
const SinglePost = ({token}) => {
  ...
  const getSimilarPost = async () => {
    try {
      const getsimilarPost = await axios.get(
        `https://tech-talk.loukteck.fr/api/user/user-post/${user_Id}`
      );
  ...
  const addToFavorite = async () => {
    try {
      const fav = await axios.put(`https://tech-talk.loukteck.fr/api/favoris/${user_Id}/${post?._id}`,{
        headers:{
  ...
  const removeToFavorite = async () => {
    try {
      const del = await axios.delete(
        `https://tech-talk.loukteck.fr/api/favoris/${user_Id}/${id}`,{
          headers:{
            Authorization: `Bearer ${token}`
          }
        }
  ...
  const deletePost = async () => {
    try {
      let del = await axios.delete(`https://tech-talk.loukteck.fr/api/post/${user_Id}/${id}`,{
        headers:{
          Authorization: `Bearer ${token}`
        }
      });
  ...
  useEffect(() => {
    const getPost = async () => {
      try {
        ...
        const getPost = await axios.get(`https://tech-talk.loukteck.fr/api/post/${id}`,{
          headers:{
          Authorization: token
        }} );
  ...
  useEffect(() => {
    const getFavorisId = async () => {
      try {
        let { data } = await axios.get(`https://tech-talk.loukteck.fr/api/favoris/${user_Id}/favoris-ids`,{
          headers:{
          Authorization: token
        }} );
  ...
  return (
    ...
    <div className="singlePage container position relative">
      <button className="btn btn-success scrollToTop" onClick={goToTop}>top</button>
      {loading ? (
        <SinglePageLoading />
      ) : (
        <>
          ...
          <div className="user_group text-dark my-4 d-flex justify-content-between align-items-center">
            <div className="user d-inline-flex align-items-center">
              {post?.user?.photo ? (
                <img src={`https://tech-talk.loukteck.fr/assets/profile/${post?.user?.photo}`} alt="user"  style={{ width:"50px" }} height="50px" className="user-img object-fit-cover rounded-pill" />
              ) : (
          ...
          <div className="post-img">
            {post?.picture && (
              <img
                src={`https://tech-talk.loukteck.fr/assets/posts/${post?.picture}`}
                alt={post?.title}
