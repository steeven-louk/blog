...
export const Sidebar = () => {
        ...
        if (result.isConfirmed) {
          try {
            let del = await axios.delete(
              `https://tech-talk.loukteck.fr/api/user/delete/${id}`
            );
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        const user = await axios.get("http://https://tech-talk.loukteck.fr/api/user/" + id, {
          headers: {
            Authorization: token,
          },
        });
        ...
        <div className="menu-list">
          <div className="user card bg-dark shadow-lg p-2">
            <div className="img_container">
              {userData?.photo ? (
                <img
                  src={`http://https://tech-talk.loukteck.fr/assets/profile/${userData?.photo}`}
