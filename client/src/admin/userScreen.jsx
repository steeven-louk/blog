...
export const UserScreen = ({token}) => {
    ...
    const getAllUsers =async () =>{
      ...
      const getUsers = await axios.get("https://tech-talk.loukteck.fr/api/admin/users", {
        headers:{
...
if (result.isConfirmed) {
  try {
      let del = await axios.delete(`https://tech-talk.loukteck.fr/api/user/delete/${userId}`,{
        headers:{
          Authorization: `Bearer ${token}`
        }
      });
            ...
            <table className="table">
              ...
              {users?.map((item, index)=>(
                <tr key={item?._id} >
                  ...
                  <td>
                  {item?.photo ? 
                  <img src={`https://tech-talk.loukteck.fr/assets/profile/${item?.photo}`} alt="card-img" style={{ "width":"50px", "height": "50px" }}  className="objectFit-cover rounded-pill" />
                  : 
